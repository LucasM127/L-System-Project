...
;A(50)
Productions:
K(x) : x >50 => /(137)[^(80)A(70-x)]K(x-1)
K(x) : x<=50 && x >=20=> B/(137)[^(80)A(20)]K(x-1)
...
;is that a concern?
;mutual exclusivity. How to test?  Pattern recognition? if else in the syntax? :(
;as is has to check all of the conditions and realize only one is valid.  (as long as both are mutually exclusive)
;LHS > A X LHS <= A
